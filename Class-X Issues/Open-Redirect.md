...
### Google Dork And FUzz For Parameters Like
```bash
redirect_url
suceess_url
cancel_url
...
```
#### Exploit
- Use Double Url encoding and tripple url encoding
- Redirect To Any Ip address instead of a domain name
-  Try XSS
```bash
...
```
- For open redirects, try using this character:
```bash
 。
```
Usage:
```bash
?url=//google。com Goes to: https://google.com URL encoded: %E3%80%82 
```
#### Intruder 
```bash
<>//Ⓛ𝐨𝗰�𝕝ⅆ𝓸ⓜₐℹⓃ｡Ｐⓦ
